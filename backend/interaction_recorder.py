...
class InteractionRecorder:
    async def record(self, interaction: InteractionCreate):
        ...
        inserted = await config.supabase.table("interactions").insert(new_interaction).execute()

        
        # 2. class_students 테이블의 total_questions 카운트 업
        # supabase rpc 나 select 후 update 방식
        student_record = await config.supabase.table("class_students").select("*").eq("class_id", interaction.class_id).eq("student_id", interaction.student_id).execute()

        
        if student_record.data:
            current_count = student_record.data[0].get("total_questions", 0)
            await config.supabase.table("class_students").update({"total_questions": current_count + 1}).eq("id", student_record.data[0]["id"]).execute()

            
        return inserted.data[0] if inserted.data else None

    async def get_page_heatmap(self, class_id: str):
        if config.supabase is None:
            return []
    ...
    async def get_hot_pages(self, class_id: str, top_n: int = 5):
        heatmap = await self.get_page_heatmap(class_id)
