...
def get_quizzes_for_student(class_id: str, student_id: str):
    """
    ...
    for q in res.data:
        if q["quiz_type"] == "common" or q.get("target_student_id") == student_id:
            # 하위 질문 목록 가져오기
            q_res = supabase.table("quiz_questions").select("*").eq("quiz_id", q["id"]).execute()
            q["questions"] = q_res.data
            quizzes.append(q)
            
    return quizzes

def evaluate_answer(question_id: str, student_id: str, submitted_answer: str):
    """
    답안 제출 및 채점, 결과 리턴
    """
    # 1. 문제 정보 조회
    q_res = supabase.table("quiz_questions").select("*").eq("id", question_id).execute()
    if not q_res.data:
    ...
    # 이미 푼 기록이 있는 확인
    exist_res = supabase.table("quiz_submissions").select("id").eq("question_id", question_id).eq("student_id", student_id).execute()
    if exist_res.data:
        submission_id = exist_res.data[0]["id"]
        supabase.table("quiz_submissions").update(submission_data).eq("id", submission_id).execute()
    else:
